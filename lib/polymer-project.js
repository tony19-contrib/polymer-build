...
class HtmlSplitter extends stream_1.Transform {
    ...
    _splitScript(file, doc, filePath) {
    ...
    _splitStyle(file, doc, filePath) {
        ...
        styleTags.forEach((styleTag, i) => {
            ...
            const childFilename = `${osPath.basename(filePath)}_style_${i}.css`;
            const childPath = osPath.join(osPath.dirname(filePath), childFilename);
...
HtmlSplitter.isInlineStyle = pred.hasTagName('style');
/**
...
class HtmlRejoiner extends stream_1.Transform {
    ...
    _rejoinStyle(splitFile, doc) {
        ...
        for (const linkTag of linkTags) {
            ...
            const stylePath = osPath.join(osPath.dirname(splitFile.path), hrefAttribute);
            if (splitFile.parts.has(stylePath)) {
                const styleSource = splitFile.parts.get(stylePath);
                // Replace <link> with equivalent <style> tag
                const styleTag = treeAdapter.createElement('style', linkTag.namespaceURI, []);
...
HtmlRejoiner.isExternalStyle = pred.AND(pred.hasTagName('link'), pred.hasAttr('href'), pred.hasAttrValue('rel', 'stylesheet'), pred.hasAttr('data-split'));
