...
class BuildAnalyzer {
    ...
    startAnalysis() {
        ...
        this._dependenciesProcessingStream =
            this._dependenciesStream.pipe(new streams_1.VinylReaderTransform())
    ...
    _done() {
        ...
        if (errorWarningCount > 0) {
            const err = new Error(`${errorWarningCount} error(s) occurred during build.`);
        ...
        if (this.loader.hasDeferredFiles()) {
            ...
            const err = new Error(this.loader.deferredFiles.size + ` deferred files were never loaded`);
            this._sourcesProcessingStream.emit('error', err);
            this._dependenciesProcessingStream.emit('error', err);
            return;
        }
